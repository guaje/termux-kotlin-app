...
If `com.termux` was installed from F-Droid, Google Play, another fork, or a debug/test-key release, uninstall that app before installing this release. This is a one-time migration; subsequent releases from this repository must retain the production signing key first used for v2.5.0.
